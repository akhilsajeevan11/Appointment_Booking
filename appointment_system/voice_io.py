...
# # Define callbacks for debugging/logging
...
# if __name__ == "__main__":
#     print("Starting ElevenLabs voice conversation. Press Ctrl+C to exit.")
#     try:
#         conversation.start()
#     except KeyboardInterrupt:
...
# appointment_system/voice_io.py
...
class ElevenLabsSpeechToTextHandler:
    ...
    def listen_and_transcribe(self, duration=7):
        print(f"Press Enter to start recording for up to {duration} seconds...")
        input()
        print("Recording...")
        ...
        # ElevenLabs STT API call using SDK
        try:
            with open(wav_path, "rb") as audio_file:
                transcription = self.client.speech_to_text.convert(
                    file=audio_file,
                    model_id="scribe_v1",  # Use the valid model_id
                    diarize=True,           # Optional: enable speaker diarization
                    # language_code="eng", # Optional: specify language or let it auto-detect
                )
            text = transcription.get("text", "").strip() if isinstance(transcription, dict) else str(transcription).strip()
...
class ElevenLabsTextToSpeechHandler:
    def __init__(self, voice="Rachel"):
        self.client = ElevenLabs(api_key=os.getenv("ELEVENLABS_API_KEY"))
        self.voice = os.getenv("ELEVENLABS_VOICE_ID")
